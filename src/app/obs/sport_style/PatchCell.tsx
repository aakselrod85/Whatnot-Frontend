...
export interface PatchStyle {
    ...
    // 'classic' draws the logo exactly as before. 'crisp' (A/B test) loads a pre-resized logo file
    // matched to the drawn size (scripts/make-board-logos.mjs) and replaces the soft contact shadow
    // with a hard outline, so the logo edge survives stream compression.
...
// crisp mode: the pre-resized file for a logo drawn `px` wide — the smallest generated size that is
// at least `px`, so the browser only ever shrinks it a little. Falls back to the original for
// anything make-board-logos.mjs doesn't cover (non-team art, or larger than the biggest size).
...
export default function PatchCell({ background: paletteBackground, stitch, logoSrc, style: s, sold, size, label, edgeTier }: Props) {
    ...
    // crisp escapes only spaces (see the <img> srcSet below) so an already-encoded path from the
    // playground isn't double-encoded; classic keeps its original encodeURI.
    const maskUrl = crisp
    ...
    return (
        ...
        >
            ...
            {label ? (
                ...
                <img
                    src={crisp ? boardLogoSrc(logoSrc, logoPx) : logoSrc}
                    ...
                    srcSet={crisp ? `${boardLogoSrc(logoSrc, logoPx).replace(/ /g, '%20')} 1x, ${boardLogoSrc(logoSrc, logoPx * 2).replace(/ /g, '%20')} 2x` : undefined}
                    alt=""
                    style={{
                        position: 'absolute',
                        ...logoBox(size, s.logoScale, s.logoLift),
